...
Поэтому временная сложность в среднем случае получается N*Log(N), кол-во обработок на одном уровне помноженное на колличество уровней.
В худжем случае алгоритм деградирует в N*N или N^2 при постоянном неудачном выборе пивота, что привело бы к глубине рекурсии N.
Но на практике такого не происходит, поэтому принято считать временную сложность данного алгоритма N*Log(N)
